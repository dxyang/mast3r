...
def save_points(params, frame, folder, calibs, tracks, images, uint16_images = False):
    only_good = params.only_good
    ...
    with open(folder + "sparse/0/points3D.bin", "wb") as fout:
        ...
        for (track_id, [points, good_prjs, bad_prjs]) in tracks.items():
            ...
            if uint16_images:
                scaled_color = tuple(int(value / 65535 * 255) for value in track.color)
            else:
                scaled_color = track.color
            fout.write(u64(track_id))
            fout.write(d64(pt.x))
            fout.write(d64(pt.y))
            fout.write(d64(pt.z))
            fout.write(u8(scaled_color[0]))
            fout.write(u8(scaled_color[1]))
            fout.write(u8(scaled_color[2]))
            fout.write(d64(0))
...
class ExportSceneParams():
    ...
    def log(self):
        ...
        print("Uint16 input images?:", self.uint16_images)
...
def export_for_gaussian_splatting(params = ExportSceneParams(), progress = QtWidgets.QProgressBar()):
    ...
    for chunk_id, (chunk_key, chunk_folder) in enumerate(chunk_dirs.items()):
        ...
        for frame_id, frame in enumerate(chunk.frames):
            ...
            print(f"saving points! uint16: {params.uint16_images}")
            save_points(params, frame, folder, calibs, tracks, images, params.uint16_images)
...
class ExportSceneGUI(QtWidgets.QDialog):
    ...
    def run_export(self):
        ...
        params.uint16_images = self.uint16ImagesBox.isChecked()
        try:
    ...
    def __init__(self, parent):
        ...
        self.uint16ImagesTxt = QtWidgets.QLabel()
        self.uint16ImagesTxt.setText("Images in uint16")
        self.uint16ImagesTxt.setFixedSize(100, 25)

        self.uint16ImagesBox = QtWidgets.QCheckBox()
        self.uint16ImagesBox.setChecked(defaults.uint16_images)
        ...
        uint16ImagesToolTip = "If image colors are stored as uint16, check this so point cloud exported will be converted properly to uint8"
        self.uint16ImagesTxt.setToolTip(uint16ImagesToolTip)
        self.uint16ImagesBox.setToolTip(uint16ImagesToolTip)
        ...
        advanced_layout.setSpacing(9)
        advanced_layout.addWidget(self.uint16ImagesTxt, 2, 0)
        advanced_layout.addWidget(self.uint16ImagesBox, 2, 1)
        ...
        self.buttons = [self.btnP1, self.btnQuit, self.radioBtn_allC, self.radioBtn_selC, self.radioBtn_allF, self.radioBtn_selF, self.zcxyBox, self.locFrameBox, self.imgQualSpBox, self.expImagesBox, self.expMasksBox, self.uint16ImagesBox]
